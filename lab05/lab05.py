...
class LinkedList:
    ...
    def __getitem__(self, idx):
        ...
        if self.length == 0:
            raise IndexError
        actualidx = self._normalize_idx(idx)
        i = 0
        n = self.head.next
        while (i < actualidx):
            if (n.next.val == None):
    ...
    def __str__(self):
        ...
        if (self.length == 0):
            ...
            while (i < self.length - 1):
                stringrep = stringrep + str(n.val) + ', '
                n = n.next
                i += 1
            stringrep = stringrep + str(self.head.prior.val) + ']'
        return stringrep
        ### END SOLUTION
    ...
    def insert(self, idx, value):
        ...
        if idx == self.length:
            ...
            """for i in range(0, idx):
                if k.next.val == None:
                    raise IndexError
                k = k.next"""
        n = LinkedList.Node(value, prior=k.prior, next=k)
        k.prior.next = n
        k.prior = n
        self.length += 1
            ...
            #raise IndexError
        """if(self.length == 0):
            n = LinkedList.Node(value, prior=self.head, next=self.head)
            self.head = n
            self.length += 1
            return self.head"""
        """if (idx == self.length):
        ...
        """if(idx == 0):
            ...
            while(i < idx):
                n = n.next
                i += 1
            k = LinkedList.Node(value, prior=n.prior, next=n)
            n.prior.next = k
            n.prior = k
            return k"""
        ### END SOLUTION
    ...
    def __add__(self, other):
        ...
        return x
        """y = other.copy()
        x.head.prior.next = y.head.next
        y.head.prior = x.head.prior
        y.head.prior.next = x.head.next
        x.head.prior = y.head.prior
        return x #may need to create copies of lists"""
        ### END SOLUTION
    ...
    def extend(self, other):
        ...
        while (i < len(other)):
            self.append(other[i])
            i += 1
        """self.head.prior.next = other.head
        other.head.prior = self.head.prior
        other.head.prior.next = self.head
        self.head.prior = other.head.prior"""
        return self
        ### END SOLUTION
